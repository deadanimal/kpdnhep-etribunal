...
function validateCertificate($path,$sign, $toSign)
{
	...
	$fpxcert=array($path."fpxuat.cer",$path."fpxuat.cer");
	...
	if(count($certs)==1)
	{
	   ...
	   $pkeyid =openssl_pkey_get_public($certs[0]);
	      ...
	      if($ret!=1) 
	      {
	  	   $ErrorCode=" Your Data cannot be verified against the Signature. "." ErrorCode :[09-1]";
	  	   return "09";	  
	      }
    }
	 ...
	 $pkeyid =openssl_pkey_get_public($certs[0]);
	   ...
	   if($ret!=1)
	   {
	    ...
	    $pkeyid =openssl_pkey_get_public($certs[1]);
         ...
         if($ret!=1) 
	     {
		  $ErrorCode=" Your Data cannot be verified against the Signature. "." ErrorCode :[09-2]";
		  return "09";	  
	      }
	    }
