                ...
                <div class="form-horizontal form-bordered ">
                    <div class="form-body">
                        <div class="form-group" style="display: flex;">
                            <div class="control-label col-xs-4" style="padding-top: 13px;">{{ trans('form11.f11_filing_date') }}</div>
                            <div class="col-xs-8 font-green-sharp" style="align-items: stretch;">
                                <span id="filing_date">
                                 {{ date('d/m/Y', strtotime($form11->created_at)) }}
                                </span>
                            </div>
                        </div>
                        <div class="form-group" style="display: flex;">
                            <div class="control-label col-xs-4" style="padding-top: 13px;">{{ trans('new.hearing_date') }}</div>
                            <div class="col-xs-8 font-green-sharp" style="align-items: stretch;">
                                <span id="hearing_date">{{ date('d/m/Y', strtotime($form11->form4->hearing->hearing_date)) }}
                                </span>
                            </div>
                        </div>    
                    </div>
    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption font-dark">
                ...
                <span class="caption-subject bold uppercase"> {{ trans('form11.list_witness') }} </span>
            </div>
            <div class="tools"> </div>
        </div>
...
<script type="text/javascript">
    ...
    $('#modalperanan').on('hidden.bs.modal', function(){
     ...
     function exportPDF(id) {
        location.href = "{{ url('') }}/form11/"+id+"/export/pdf";
    }
    function exportDOCX(id) {
        location.href = "{{ url('') }}/form11/"+id+"/export/docx";
    }
    ...
    var TableDatatablesButtons = function () {
        ...
        var oTable = table.DataTable({
            ...
            "columns": [
                ...
                { data: "witness_on_behalf", name:"witness_on_behalf"},
                { data: "witness_name", name:"witness_name"},
            ...
            buttons: [
             {
                text:"<i class=\"fa fa-plus margin-right-5\"></i>  {{ __('form11.reg_witness')}} ", className:"btn blue btn-outline", action:function()
                {
                    window.location.href = "{{ route('form11-add', [$form4->form4_id]) }}"; 
                }
            },
...
function deleteWitness(id) {
    swal({
        title: "{{ trans('swal.delete_witness') }}",
                ...
                if(data.status=="ok"){
                    ...
                    function () {
                        if(data.return == 1)
                            ...
                            window.location.href = '{{ route("form11-view", [$form11_id]) }}';
                    });
