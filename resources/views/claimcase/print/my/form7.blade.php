...
<body class="center">
	...
	<div>
		...
		<div class='left'>
			<p class="justify indent">Tindakan ini telah pada hari ini dipanggil untuk pendengaran di hadapan <span class="uppercase bold">{{ $form4->president ? ( $form4->president->ttpm_data->president->salutation ? $form4->president->ttpm_data->president->salutation->salutation_my : '').' '.$form4->president->name : '' }}</span> dengan kehadiran Penentang, dan tanpa kehadiran Pihak Yang Menuntut, Tribunal dengan ini memerintahkan:</p>

			<div  style="padding-left: 30px;">
				<p class="bold justify">{!! nl2br($form4->award->award_description) !!}</p><br><br>
			</div>
			Bertarikh pada <span class='bold'>{{ date('d', strtotime($form4->award->award_date.' 00:00:00')).' '.localeMonth(date('F', strtotime($form4->award->award_date.' 00:00:00'))).' '.date('Y', strtotime($form4->award->award_date.' 00:00:00')) }}</span><br>
