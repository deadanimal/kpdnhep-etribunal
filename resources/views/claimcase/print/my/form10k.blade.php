	...
	<div>
		...
		<div class='left'>
			<p class="justify indent" >
				Tindakan ini telah didengar di hadapan <span class="bold uppercase">{{ $form4_before ? ($form4_before->president->ttpm_data->president->salutation ? $form4_before->president->ttpm_data->president->salutation->salutation_my : '').' '.$form4_before->president->name : '' }}</span> pada <span class="bold uppercase">{{ $form4_before ? date('h', strtotime($form4_before->hearing->hearing_date.' 00:00:00')).' '.localeMonth(date('F', strtotime($form4_before->hearing->hearing_date.' 00:00:00'))).' '.date('Y', strtotime($form4_before->hearing->hearing_date.' 00:00:00')) : '' }}</span> dan keputusan dibacakan oleh  <span class="bold uppercase">{{ $form4->president ? ( $form4->president->ttpm_data->president->salutation ? $form4->president->ttpm_data->president->salutation->salutation_my : '').' '.$form4->president->name : '' }}</span> pada <span class="bold uppercase"> {{ date('d', strtotime($form4->hearing->hearing_date.' 00:00:00')).' '.localeMonth(date('F', strtotime($form4->hearing->hearing_date.' 00:00:00'))).' '.date('Y', strtotime($form4->hearing->hearing_date.' 00:00:00')) }} </span> di <span class="bold uppercase">{{ $form4->hearing->hearing_room ? $form4->hearing->hearing_room->venue->hearing_venue : '-' }}</span> dengan kehadiran Pihak Yang Menuntut dan Penentang, Tribunal dengan ini memerintahkan:
			</p>

			<p class="bold justify" style="padding: 0 25px;">{!! nl2br($form4->award->award_description) !!}</p><br><br>

			<span>Bertarikh pada </span><span class='bold'>{{ date('d', strtotime($form4->hearing->hearing_date.' 00:00:00')).' '.localeMonth(date('F', strtotime($form4->hearing->hearing_date.' 00:00:00'))).' '.date('Y', strtotime($form4->hearing->hearing_date.' 00:00:00')) }}</span><br>
