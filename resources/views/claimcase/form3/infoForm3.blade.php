...
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3>{{ __('form3.form3_details') }}</h3>
    <span> {{ trans('form3.form3_view') }} </span>
</div>
<div class="row">
    <div class="col-xs-12" style="text-align: right;">
        <div id='buttonDiv' class="btn-group">
        @if($claim_case_opponent->form2->form3_id)
            @if($allow)
                @if($is_staff || (!$is_staff && $claim_case_opponent->form2->form3->form_status_id < 46 && $claim_case->claimant_user_id == Auth::id()) )
                <a class="btn green-meadow btn-outline" href="{{ route('form3-edit', ['id' => $claim_case_opponent->id]) }}" style='margin-bottom: 10px; margin-left: 5px;'>
                    <i class="fa fa-edit"></i> {{ trans('button.edit') }}
                </a>
                @endif
                @if($is_staff && $claim_case_opponent->form2->form3->form_status_id != 46)
                <a class="btn purple btn-outline" href="javascript:;" onclick='processForm3({{ $claim_case_opponent->id }})' style='margin-bottom: 10px; margin-left: 5px;'>
                    <i class="fa fa-spinner"></i> {{ trans('button.process') }}
                </a>
                @endif
            @endif
            @if($claim_case_opponent->form2->form3->form_status_id == 46)
            <div class="btn-group pull-right">
                <a class="btn yellow btn-outline" href="javascript:;" style='margin-bottom: 10px; margin-left: 5px;' data-toggle="dropdown" aria-expanded="false">
                    <i class="fa fa-download"></i> {{ trans('form3.download') }}
                    <i class="fa fa-angle-down"></i>
                </a>
                <ul class="dropdown-menu">
                    <li>
                        <a href="javascript:;" onclick="exportPDF('{{ $claim_case_opponent->id }}')">
                            <i class="fa fa-file-pdf-o"></i> PDF
                        </a>
                    </li>
                    <li>
                        <a href="javascript:;" onclick="exportDOCX('{{ $claim_case_opponent->id }}')">
...
<div class="row">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            @endif @endif
            @include('claimcase.defence_counter_info3')
            @if($claim_case->form1->form2_id) @if($claim_case_opponent->form2->form_status_id == 22)
            @include('claimcase.process_info3')
            @endif @endif
...
<div class='row'>
    <div class="col-md-12" style="text-align: center;">
        <a class='btn default' href='{{ route("form3-list") }}'>{{ __('button.back_to_list') }}</a>
    </div>
</div>
@endsection
...
<script type="text/javascript">
    ...
    function exportPDF(id) {
        location.href = "{{ url('') }}/form3/"+id+"/export/pdf";
    }

    function exportDOCX(id) {
        location.href = "{{ url('') }}/form3/"+id+"/export/docx";
    }
