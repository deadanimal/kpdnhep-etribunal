    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">{{ __('new.list_suggestion')}} </span>
          </div>
          <div class="tools"> </div>
      </div>
        ...
        <table id="roles" class="table table-striped table-bordered table-hover table-responsive">
            <thead>
                <tr>
                    ...
                    <th>{{ __('new.suggested_by')}}</th>
                    <th>{{ __('new.subject')}}</th>
                    <th>{{ __('new.response')}}</th>
                    <th>{{ __('new.responded_by')}}</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('new.suggestion_info')}}</h4>
            </div>
...
<script type="text/javascript">
   ...
   /* // Initialization
    ...
    @foreach ($classifications as $classification)
        cat{{ $classification->category_id }}.push({ "id": "{{ $classification->claim_classification_id }}", "name": "{{ __('master/master_claim_classification.'.$classification->metakey) }}" });
    @endforeach
    ...
    var TableDatatablesButtons = function () {
        ...
        var initTable1 = function () {
            ...
            var oTable = table.DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('others.suggestion') }}",
                "deferRender": true,
                "pagingType": "bootstrap_full_number",
                "columns": [
                     // {data: 'rownum'},
                    { data: 'id', defaultContent: '', 'orderable' : false, 'searchable' : false},
                    { data: "created_by_user_id", name:"created_by.name"},
                    { data: "subject", name:"subject", 'orderable' : false, 'render': function ( data, type, full ) {
                        return $("<div/>").html(data).text(); 
                    }},
                    { data: "status", name:"response", 'orderable' : false, 'render': function ( data, type, full ) {
                        return $("<div/>").html(data).text(); 
                    }},
                    { data: "responded_by_user_id", name:"responded_by.name"},
                ...
                buttons: [
                    ...
                    extend: 'print',
                        ...
                        title: function(){
                            return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.list_suggestion')}}</span>"; // #translate
                        },
                        ...
                        customize: function ( win ) {
                            ...
                            win.document.title = "List of Suggestions";
    ...
    function deleteSuggestion(id) {
        swal({
            title: "{{ __('swal.delete_suggestions') }}",
            text: "{{ __('swal.sure_delete_suggestions') }}",
            type: "warning",
        ...
        function(){
            ...
            $.ajax({
                url: "{{ route('others.suggestion') }}/"+id+"/delete",
                type: "DELETE",
                data : {
                    _token:_token,
                    id:id
                ...
                success: function(data){
                    if(data.status=="ok"){
                        swal({
                            ...
                            text: "{{ __('swal.success_delete_suggestions') }}!",
                            type: "success",
