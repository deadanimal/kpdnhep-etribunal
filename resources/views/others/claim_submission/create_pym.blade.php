...
<input type="hidden" name="is_claimant_submit" value="1">
<div class="row">
    <div class="col-md-12">
        <div class="portlet light">
            ...
            <div class="portlet-body ">
                ...
                <div class="form-group form-md-line-input"">
                    <label class="col-md-4 control-label" id="description">{{ __('new.submission_category') }} :
                        <span class="required">*</span>
                    </label>
                    <div class="col-md-6">
                        <select class="form-control select2 bs-select" onchange="posReferenceNo()" id="submission_category_id" name="submission_category_id"  data-placeholder="---">
                            <option value="" disabled selected>---</option>
                            @foreach ($categories as $category)
                            <option  
                            @if( $claim_submission->submission_category_id == $category->submission_category_id) 
                            selected 
                            @endif 
                            value="{{ $category->submission_category_id }}">{{ $category->$category_lang }}</option>
                            @endforeach
                        </select>
                        <span class="help-block"></span>
                    </div>
                </div>
                <div id="row_pos_ref_no" class="form-group form-md-line-input @if( $claim_submission->submission_category_id != 2 ) hidden @endif">
                    <label for="cost" class="control-label col-md-4">{{ __('new.pos_ref_no') }} :
                        <span class="required"> * </span>
                    </label>
                    <div class="col-md-6">
                        <input type="text" class="form-control" id="pos_reference_no" name="pos_reference_no" value="{{ $claim_submission->pos_reference_no}}"/>
                        <span class="help-block"></span>
                    </div>
                </div>

                <div class="form-group form-md-line-input">
                    <label for="category" class="control-label col-md-4">{{ __('new.submission_type') }} :
                        <span class="required">*</span>
                    </label>
                    <div class="col-md-6">
                        <select class="form-control select2 bs-select" id="submission_type_id" name="submission_type_id"  data-placeholder="---">
                            <option value="" disabled selected>---</option>
                            @foreach ($types as $type)
                            <option
                            @if( $claim_submission->submission_type_id == $type->submission_type_id)  selected 
                            @endif
                            value="{{ $type->submission_type_id }}">{{ $type->$type_lang }}</option>
                            @endforeach
                        </select>
                        <span class="help-block"></span>
                    </div>
...
<script type="text/javascript">
    ...
    function posReferenceNo() {
        var category = $("#submission_category_id").val();

        console.log(category);

        if(category == 2)
            $("#row_pos_ref_no").removeClass("hidden");
        else
            $("#row_pos_ref_no").addClass("hidden");
    }
