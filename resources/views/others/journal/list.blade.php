    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">{{ __('others.journal_list') }}</span>
          </div>
          <div class="tools"> </div>
        </div>
            ...
            <form method='get' action=''>
                <div id="search-form" class="form-inline">
                    <div class="form-group mb10">
                        <label for="claim_category_id">{{ __('others.category') }}</label>
                    ...
                    <div class="form-group mb10">
                        <label for="claim_classification_id">{{ __('others.classification') }}</label>
                        <select class="form-control select2 bs-select" id="claim_classification_id" name="claim_classification_id"  data-placeholder="---">
                            <option value="" selected disabled hidden>-- {{ __('others.all_classification') }} --</option>
                        </select>
                    </div>
                    <br class='hidden-xs hidden-sm'>
                    <div class="form-group mb10">
                        <label for="status">{{ __('new.status') }}</label>
                        <select class="form-control select2 bs-select" id="status" name="status"  data-placeholder="---">
                            <option value="" >-- {{ __('form1.all_status') }} --</option>
                            <option @if(Request::get('status') == 1 ) selected @endif value="1"> {{ __('new.publish') }} </option>
                            <option @if(Request::get('status') == 2 ) selected @endif value="2"> {{ __('new.unpublish') }} </option>
                        </select>
                    </div>
                    <div class="form-group mb10">
        ...
        <div class="portlet-body">
            <table id="roles" class="table table-striped table-bordered table-hover table-responsive">
                <thead>
                    <tr>
                        <th width="3%">{{ __('others.no') }}</th>
                        <th>{{ __('new.claim_no') }}</th>
                        <th>{{ __('others.category') }}</th>
                        <th>{{ __('others.classification') }}</th>
                        <th>{{ __('others.upload_date') }}</th>
                        <th>{{ __('new.status') }}</th>
                        <th>{{ __('others.action') }}</th>
                    </tr>
                </thead>
            </table>
        </div>
...
<div class="modal fade bs-modal-lg" id="modaljournal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                <h4 class="modal-title">{{ __('others.journal_info') }}</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" role="form" method="post" action="{{ route('others.journal.createclaim') }}" id="createclaim">
                    <div class="form-group form-md-line-input">
                            <label class="col-md-4 control-label" id="description"> {{ __('new.claim_no')}} :<span class="required">*</span></label>
                            <div class="col-md-6">
                                <input required="" type="text" class="form-control" id="case_no" name="case_no">
                                <br/>
                                <a id="btn-check-case" class="btn btn-primary"> {{ __('others.create_journal')}}</a>
                            </div>
                    </div>
                </form>
            </div>
...
<script type="text/javascript">
    ...
    $('#btn-check-case').click(function(){
        var case_no = $('#case_no').val();
        $.post('{{ route('others.journal.checkcase') }}',
        {
            case_no: case_no
        },
        function(data){
        ...
        var initTable1 = function () {
            ...
            var oTable = table.DataTable({
                ...
                "columns": [
                    ...
                    { data: "journal_desc", name:"journal_desc"},
                    { data: "claim_category_id", name:"claim_category_id"},
                    { data: "claim_classification_id", name:"claim_classification_id"},
                ...
                buttons: [
                    {
                        text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ __('others.add_journal') }}", className:"btn blue btn-outline btn-create-journal", action:function()
                            {
                                //window.location.href = "{{ route('others.journal.create') }}";
                    ...
                    },
                    {
                        ...
                        title: function(){
                            return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('others.journal_list') }}</span>"; // #translate
                        },
                        ...
                        customize: function ( win ) {
                            ...
                            win.document.title = "{{ __('others.journal_list') }}";
    ...
    function deleteJournal(id) {
        ...
        function(){
            ...
            $.ajax({
                url: "{{ route('others.journal') }}/"+id+"/delete",
                type: "DELETE",
                data : {
                    _token:_token,
                    id:id
    ...
    @if(Request::has('claim_category_id'))
        $('#claim_category_id').val({{Request::get('claim_category_id')}}).trigger('change');

        @if(Request::has('claim_classification_id'))
            $('#claim_classification_id').val({{Request::get('claim_classification_id')}}).trigger('change');
        @endif
    @endif
</script>
