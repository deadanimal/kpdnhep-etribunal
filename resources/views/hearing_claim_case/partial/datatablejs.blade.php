        ...
        var @if(!empty($customname)) {{$customname}} @else Datatable @endif =  $('{{ $table }}').DataTable({
            ...
            @if(!empty($columns))
                ...
                @foreach ($columns as $data => $name)
                    @if($data == 'matured_date')
                        {data : '{{ $data }}', @if(!empty($name))name : '{{ $name }}',@endif orderable: true, searchable: true, 'render': function ( data, type, full ) {
                            return $("<div/>").html(data).text(); 
                        }},
                    @elseif($data == 'case_no')
                        {data : '{{ $data }}', @if(!empty($name))name : '{{ $name }}',@endif orderable: true, searchable: true},
                    @else
                        {data : '{{ $data }}', @if(!empty($name))name : '{{ $name }}',@endif orderable: false, searchable: false},
                    @endif
                @endforeach
            ...
            @if($buttons==true)
                ...
                @endif
                {
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ trans('menu.claim_no_hearing_date') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ trans('menu.claim_no_hearing_date') }}";
