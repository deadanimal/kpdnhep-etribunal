    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">{{ __('master.list_category')}} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        <table id="tableCategory" class="table table-striped table-bordered table-hover table-responsive">
            <thead>
              <tr>
                <th width="3%">{{ __('master.no')}}</th>
                <th>{{trans('master.category_en')}}</th>                
                <th>{{trans('master.category_my')}}</th>
                <th>{{ __('master.category_code')}}</th>
    ...
    <div class="modal-dialog modal-lg"> 
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('master.info_category')}}</h4>
            </div>
...
<script type="text/javascript">
    var TableDatatablesButtons = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.category') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                // {data: 'rownum'},
                { data: "claim_category_id", name: "claim_category_id", searchable: false},
                { data: "category_en", name:"category_en", 'orderable' : false},
                { data: "category_my", name:"category_my", 'orderable' : false},
                { data: "category_code", name:"category_code", 'orderable' : false},
            ...
            buttons: [
            {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('master.add_category') }} ", className:"btn blue btn-outline", action:function()
                {
                    window.location.href = "{{ route('master.category.create') }}"; 
                }
            },
            
            {
                ...
                title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.list_category') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('new.list_category') }}";
...
function deleteCategory(id) {
    swal({
        title: "{{ trans('swal.delete_category') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.category') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateCategory(id) {
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.category') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
