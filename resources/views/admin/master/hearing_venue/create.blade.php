...
{{ Form::open(['route' => 'master.hearing_venue.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.hearing_venue.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="hearing_venue_id" value="{{ $hearing_venue->hearing_venue_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3>{{ __('master.hearing_venue')}}</h3>
        ...
        <div class="portlet light">
            <div class="portlet-title">
                <div class="caption font-dark">
                    <span class="caption-subject font-green-sharp bold uppercase">{{ __('master.hearing_venue')}} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body ">
                ...
                {{ textInput($errors, 'text', $hearing_venue, 'hearing_venue', __('master.hearing_venue'), true) }}

                {{ masterSelect($errors, $branches, $hearing_venue, 'branch_id', 'branch_id', 'branch_name', __('master.branch_name'), true) }}
            ...
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.hearing_venue') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status=='ok'){
                ...
                function () {
                    window.location.href = "{{ route('master.hearing_venue') }}";
                });
            } else {
                var inputError = [];
