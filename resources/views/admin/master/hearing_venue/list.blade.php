    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption font-dark">
                ...
                <span class="caption-subject bold uppercase">{{ __('master.list_hearing_venue') }}</span>
            </div>
            <div class="tools"> </div>
        </div>
        ...
        <div class="portlet-body">
            <table id="hearing_venue" class="table table-striped table-bordered table-hover table-responsive">
                <thead>
                    <tr>
                        <th width="3%">{{ trans('hearing.no')}}</th>
                        <th>{{ __('master.hearing_venue')}}</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('master.hearing_venue_info')}}</h4>
            </div>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        ...
        var oTable = table.DataTable({
            ...
            "columns": [
                 // {data: 'rownum'},
                { data:"hearing_venue_id", name:"hearing_venue_id", searchable: false},
                { data:"hearing_venue", name:"hearing_venue"},
            ...
            buttons: [
                {
                    text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ __('master.hearing_venue') }} ", className:"btn blue btn-outline", action:function()
                        {
                            window.location.href = "{{route('master.hearing_venue.create')}}";
                        }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.list_hearing_venue') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('master.list_hearing_venue') }}";
...
function deleteHearingVenue(id) {
    swal({
        title: "{{ __('swal.delete_hearing_venue') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.hearing_venue') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateHearingVenue(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.hearing_venue') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
