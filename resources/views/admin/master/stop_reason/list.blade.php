    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase"> {{ __('master.stop_reason') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        ...
        <thead>
          <tr>
            ...
            <th>{{ trans('master.stop_reason') }} (EN)</th>            
            <th>{{ trans('master.stop_reason') }} (MY)</th>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.stop_reason') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // {data: 'rownum'},
                { data: "stop_reason_id", name:"stop_reason_id", searchable: false},
                { data: "stop_reason_en", name:"stop_reason_en", 'orderable' : false},
                { data: "stop_reason_my", name:"stop_reason_my", 'orderable' : false},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ __('master.add_stop_reason') }}", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.stop_reason.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.stop_reason') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('master.stop_reason') }}";
...
function deleteStopReason(id) {
    swal({
        title: "{{ __('swal.delete_stop_reason')}}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.stop_reason') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateStopReason(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.stop_reason') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
