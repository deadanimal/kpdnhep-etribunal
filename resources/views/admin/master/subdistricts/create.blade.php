...
{{ Form::open(['route' => 'master.submission_type.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.submission_type.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="submission_type_id" value="{{ $submission_type->submission_type_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3> {{ trans('master.submission_type') }} </h3>
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-green sbold uppercase">{{ trans('master.submission_type') }} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
            	...
            	{{ textInput($errors, 'text', $submission_type, 'type_en', trans('new.type').' (EN)', true) }}

                {{ textInput($errors, 'text', $submission_type, 'type_my', trans('new.type').' (MY)', true) }}


            </div>
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.submission_type') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status=='ok'){
                ...
                function () {
                    window.location.href = '{{ route('master.submission_type') }}';
                });
            } else {
                var inputError = [];
