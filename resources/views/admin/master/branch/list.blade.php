    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase"> {{ trans('new.list_branch') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        <table id="tableBranch" class="table table-striped table-bordered table-hover table-responsive">
        <thead>
          <tr>
            <th>{{ trans('new.no') }}</th>
            <th>{!! trans('master.branch_name_list') !!}</th>
            <th>{!! trans('master.branch_code_list') !!}</th>
            <th style="width: 40%">{{ trans('new.branch_address') }}</th>
            <th>{{ trans('new.branch_contact') }}</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ trans('new.branch_view') }}</h4>
            </div>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        ...
        var oTable = table.DataTable({
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('new.add_branch') }} ", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.branch.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.list_branch') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('new.list_branch') }}";
...
function deleteBranch(id) {
    swal({
        title: "{{ trans('swal.branch_delete') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.branch') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateBranch(id) {
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.branch') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
