...
@if(strpos(Request::url(),'federal/edit') !== false)
{{ Form::open(['route' => 'master.holiday.federal.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submit']) }}
@else
{{ Form::open(['route' => 'master.holiday.federal.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submit']) }}
@endif
                ...
                <div class="col-md-12 form">
                    <div class="form-body">
                        <div class="form-group form-md-line-input">
                            ...
                            <div class="col-md-5">
                                <select id="event" class="form-control select2 bs-select" name="event" style="margin-right: 10px;">
                                    <option value=""> -- {{ __('new.all') }} --</option>
                                    @foreach($events as $i=>$event)
                                    <option 
                                    @if( $holiday->holiday_event_id == $event->holiday_event_id) selected 
                                    @endif 
                                    value="{{ $event->holiday_event_id }}">{{ $event->$event_lang }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
function submit(){
    ...
    $.ajax({
        ...
        @if(strpos(Request::url(),'federal/edit') !== false)
            url: "{{ route('master.holiday.federal.update') }}",
        @else
            url: "{{ route('master.holiday.federal.store') }}",
        @endif
        type: 'POST',
        data: $('form').serialize(),
        datatype: 'json',
