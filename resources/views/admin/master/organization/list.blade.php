    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">{{ __('master.list_org') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
        ...
        <table id="tableorganization" class="table table-striped table-bordered table-hover table-responsive">
            <thead>
              <tr>
                ...
                <th>{{ __('master.organization') }}</th>
                <th>{{ __('master.org_desc') }}</th>
                <th>{{ __('master.org_address') }}</th>
    ...
    <div class="modal-dialog modal-lg"> 
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('master.organization_details') }}</h4>
            </div>
...
<script type="text/javascript">
    var TableDatatablesButtons = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.organization') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // // {data: 'rownum'},
                 { data: "jurisdiction_organization_id", name: "jurisdiction_organization_id", searchable: false},
                 { data: "organization", name: "organization", 'orderable': false},
                 { data: "org_description", name:"org_description", 'orderable' : false},
            ...
            buttons: [
            {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ __('master.add_org')}} ", className:"btn blue btn-outline", action:function()
                {
                    window.location.href = "{{ route('master.organization.create') }}"; 
                }
            },
            {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.list_org') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('master.list_org') }}";
...
function deleteOrganization(id) {
    swal({
        title: "{{ __('swal.delete_organization')}}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.organization') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateOrganization(id) {
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.organization') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
