...
{{ Form::open(['route' => 'master.organization.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.organization.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif

<input type="hidden" name="jurisdiction_organization_id" value="{{ $organization->jurisdiction_organization_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3>{{ __('master.organization')}}</h3>
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-green sbold uppercase">{{ __('master.organization_details')}} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
            	{{ textInput($errors, 'text', $organization, 'organization', __('master.org_name') , true) }}
            	{{ textInput($errors, 'text', $organization, 'org_description', trans('master.org_desc')) }}
                {{ textInput($errors, 'text', $organization, 'org_address', trans('master.org_address'),true ) }}
                {{ textInput($errors, 'text', $organization, 'org_address2', '') }}
                {{ textInput($errors, 'text', $organization, 'org_address3', '') }}
                {{ textInputNumeric($errors, 'text', $organization, 'org_postcode', trans('master.postcode'),true ) }}
                {{ masterSelect($errors, $states, $organization, 'org_state_id', 'state_id', 'state', trans('new.state'), true) }}
                {{ masterSelect($errors, false, $organization, 'org_district_id', 'district_id', 'district', trans('new.district'), true) }}
                {{ textInputNumeric($errors, 'text', $organization, 'org_office_phone', trans('master.office_ph'), true) }}
                {{ textInputNumeric($errors, 'text', $organization, 'org_office_fax', trans('master.office_fax')) }}
                {{ textInput($errors, 'text', $organization, 'org_email', trans('master.email') ) }}
            </div>

            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.organization') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
...
<script type="text/javascript">
    ...
    @include('components.js.ajaxdistrict',[
        ...
        'state_id' => '#org_state_id',
        'inputEmpty' => $organization->org_district_id
        ])
        ...
        $.ajax({
            ...
            success: function(data) {
                if(data.status=='ok'){
                    ...
                    function () {
                        window.location.href = '{{ route('master.organization') }}';
                    });
                } else {
                    var inputError = [];
