...
{{ Form::open(['route' => 'master.offence_type.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.offence_type.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif

<input type="hidden" name="offence_type_id" value="{{ $offence->offence_type_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3>{{ __('master.offence_type')}}</h3>
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-green sbold uppercase"> {{ __('master.offence_type')}} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
            	{{ textInput($errors, 'text', $offence, 'offence_code', __('master.offence_code'), true) }}
            	{{ textInput($errors, 'text', $offence, 'offence_description_en', __('master.desc_en'), true ) }}
                {{ textInput($errors, 'text', $offence, 'offence_description_my', __('master.desc_my'), true ) }}
            </div>
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.offence_type') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status=='ok'){
                ...
                function () {
                    window.location.href = '{{ route('master.offence_type') }}';
                });
            } else {
                var inputError = [];
