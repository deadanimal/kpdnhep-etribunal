    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">{{ __('master.list_offence')}} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        <table id="tableoffence" class="table table-striped table-bordered table-hover table-responsive">
            <thead>
              <tr>
                <th width="3%">{{ __('master.no')}}</th>
                <th>{{ __('master.offence_code')}}</th>
                <th width="35%">{{ __('master.desc_en')}}</th>
                <th width="36%">{{ __('master.desc_my')}}</th>
    ...
    <div class="modal-dialog modal-lg"> 
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('master.offence_info')}}</h4>
            </div>
...
<script type="text/javascript">
    var TableDatatablesButtons = function () {
        ...
        var initTable1 = function () {
            var table = $('#tableoffence');

        //DB BASE
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.offence_type') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // {data: 'rownum'},
                 { data: "offence_type_id", name: "offence_type_id", searchable: false},
                 { data: "offence_code", name:"offence_code", 'orderable' : false},
                 { data: "offence_description_en", name:"offence_description_en", 'orderable' : false},
                 { data: "offence_description_my", name:"offence_description_my", 'orderable' : false},
                // { data: "address", name:"address", 'orderable' : false, render: function(data, type, row){
                //     return $("<div/>").html(data).text();
                // }},
                // { data: "contact", name:"contact", 'orderable' : false, render: function(data, type, row){
                //     return $("<div/>").html(data).text();
                // }},
            ...
            buttons: [
            {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ __('button.register').' '.__('master.offence_type') }} ", className:"btn blue btn-outline", action:function()
                {
                    window.location.href = "{{ route('master.offence_type.create') }}"; 
                }
            },
            {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.list_offence') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('master.list_offence') }}";
                ...
                /*
                {
                    ...
                    className: 'btn dark btn-outline',
                    title: function(){
                        return "{{ trans('master.list_offence') }}";
                    },
                    exportOptions: {
                        columns: [ 0, 1, 2, 3, 4 ]
                    },
...
function deleteOffence(id) {
    swal({
        title: "{{ __('swal.delete_offence') }}",
        text: "{{ __('swal.sure_delete_offence') }}",
        type: "warning",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.offence_type') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateOffence(id) {
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.offence_type') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
