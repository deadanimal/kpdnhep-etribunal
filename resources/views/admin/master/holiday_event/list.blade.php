    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">{{ __('master.holiday_list')}} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        <table id="holiday_event" class="table table-striped table-bordered table-hover table-responsive">
            <thead>
              <tr>
                <th width="3%">{{ __('master.no')}}</th>
                <th>{{trans('master.holiday_event_name')}}</th>                
                <th>{{trans('master.holiday_date')}}</th>
    ...
    <div class="modal-dialog modal-lg"> 
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('master.info_holiday_event')}}</h4>
            </div>
...
<script type="text/javascript">
    var TableDatatablesButtons = function () {
        ...
        var initTable1 = function () {
            var table = $('#holiday_event');

        //DB BASE
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.holiday_event') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                // {data: 'rownum'},
                { data: 'id', defaultContent: '', 'orderable' : false, 'searchable' : false},
                { data: "holiday_event_name", name:"holiday_event_name", 'orderable' : false},
                { data: "holiday_date", name:"holiday_date", 'orderable' : false},
            ...
            buttons: [
            {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('master.add_event') }} ", className:"btn blue btn-outline", action:function()
                {
                    window.location.href = "{{ route('master.holiday_event.create') }}"; 
                }
            },
            {
                extend: 'print',
                ...
                title: function(){
                    return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.holiday_list') }}</span>"; // #translate
                },
                ...
                customize: function ( win ) {
                    ...
                    win.document.title = "{{ __('master.holiday_list') }}";
...
function deleteHoliday(id) {
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.holiday_event') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateHoliday(id) {
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.holiday_event') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
