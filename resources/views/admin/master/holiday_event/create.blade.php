...
{{ Form::open(['route' => 'master.holiday_event.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.holiday_event.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="holiday_event_id" value="{{ $holiday_event->holiday_event_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3> {{ trans('master.holiday_event') }} </h3>
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-green sbold uppercase">{{ trans('master.holiday_event') }} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
            	...
            	{{ textInput($errors, 'text', $holiday_event, 'holiday_event_name_en', trans('master.holiday_event_name').' EN', true) }}

                {{ textInput($errors, 'text', $holiday_event, 'holiday_event_name_my', trans('master.holiday_event_name').' MY', true) }}

            	<div class="form-group form-md-line-input">
                    <label for="holiday_date" class="control-label col-md-4">{{ __('master.holiday_date') }} :
            ...
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.category') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status=='ok'){
                ...
                function () {
                    window.location.href = '{{ route('master.holiday_event') }}';
                });
            } else {
                var inputError = [];
