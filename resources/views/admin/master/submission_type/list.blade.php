    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">{{ __('master.list_submission_type')}} </span>
          </div>
          <div class="tools"> </div>
      </div>
        ...
        <table id="tableCategory" class="table table-striped table-bordered table-hover table-responsive">
            <thead>
              <tr>
                ...
                <th>{{trans('new.type')}} (EN)</th>                
                <th>{{trans('new.type')}} (MY)</th>
    ...
    <div class="modal-dialog modal-lg"> 
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('master.info_submission_type')}}</h4>
            </div>
...
<script type="text/javascript">
    var TableDatatablesButtons = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.submission_type') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // {data: 'rownum'},
                 { data: 'id', defaultContent: '', 'orderable' : false, 'searchable' : false},
                 { data: "type_en", name:"type_en", 'orderable' : false},
                 { data: "type_my", name:"type_my", 'orderable' : false},
            ...
            buttons: [
            {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('master.add_submission_type') }} ", className:"btn blue btn-outline", action:function()
                {
                    window.location.href = "{{ route('master.submission_type.create') }}"; 
                }
            },
            {
                extend: 'print',
                ...
                title: function(){
                    return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.list_submission_type') }}</span>"; // #translate
                },
                ...
                customize: function ( win ) {
                    ...
                    win.document.title = "{{ __('master.list_submission_type') }}";
...
function deleteType(id) {
    swal({
        title: "{{ trans('swal.delete_submission_type') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.submission_type') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateType(id) {
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.submission_type') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
