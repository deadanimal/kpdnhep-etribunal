    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase"> {{ trans('master.directory_hq') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">

        <table id="tableDirectoryHQ" class="table table-striped table-bordered table-hover table-responsive">
        <thead>
          <tr>
            <th>{{ trans('new.no') }}</th>
            <th style="width: 25%;">{!! trans('master.directory_hq_name') !!}</th>
            <th style="width: 20%;">{!! trans('master.directory_hq_division') !!}</th>
            <th style="width: 15%;">{!! trans('master.directory_hq_designation') !!}</th>
            <th style="width: 5%;">{{ trans('master.directory_hq_ext') }}</th>
            <th>{{ trans('master.directory_hq_email') }}</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ trans('master.directory_hq') }}</h4>
            </div>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        var table = $('#tableDirectoryHQ');

        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.directory.hq') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // {data: 'rownum'},
                { data: 'id', defaultContent: '', 'orderable' : false, 'searchable' : false},
                { data: "directory_hq_name", name:"directory_hq_name", 'orderable' : false},
                { data: "directory_hq_division", name:"directory_hq_division", 'orderable' : false},
                { data: "directory_hq_designation", name:"directory_hq_designation", 'orderable' : false},
                { data: "directory_hq_ext", name:"directory_hq_ext", 'orderable' : false},
                { data: "directory_hq_email", name:"directory_hq_email", 'orderable' : false},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('master.add_directory') }} ", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.directory.hq.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.directory_hq') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('new.directory_hq') }}";
...
function deleteDirectoryHQ(id) {
    swal({
        title: "{{ trans('swal.directory_hq_delete') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.directory.hq') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateDirectoryHQ(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.directory.hq') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
