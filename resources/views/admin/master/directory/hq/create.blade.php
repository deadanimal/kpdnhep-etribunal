...
{{ Form::open(['route' => 'master.directory.hq.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.directory.hq.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="directory_hq_id" value="{{ $directoryHQ->directory_hq_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3> {{ trans('new.directory_hq_info') }} </h3>
        ...
        <div class="portlet light bordered">
            ...
            <div class="portlet-body">
                <div class="form-group form-md-line-input">
                    <label for="rcy_id" class="control-label col-md-4">{{ trans('master.directory_hq_division') }}
                        <span class="required"> * </span>
                    </label>
                    <div class="col-md-6">
                        <select class="form-control select2 bs-select" id="directory_hq_division" name="directory_hq_division"  data-placeholder="---">
                            <option value="" disabled selected>---</option>
                            @foreach($directoryHQ->getDivisions() as $division)
                            <option 
                            @if($directoryHQ->directory_hq_division ==  $division)
                            selected
                            @endif
                ...
                {{ textInput($errors, 'text', $directoryHQ, 'directory_hq_name', trans('master.directory_hq_name'), true) }}

                <div class="form-group form-md-line-input">
                    <label for="rcy_id" class="control-label col-md-4">{{ trans('master.directory_hq_designation') }}
                        <span class="required"> * </span>
                    </label>
                    <div class="col-md-6">
                        <select class="form-control select2 bs-select" id="directory_hq_designation" name="directory_hq_designation"  data-placeholder="---">
                            <option value="" disabled selected>---</option>
                            @foreach($directoryHQ->getDesignations() as $designation)
                            <option 
                            @if($directoryHQ->directory_hq_designation ==  $designation)
                            selected
                            @endif
                ...
                {{ textInput($errors, 'text', $directoryHQ, 'directory_hq_ext',trans('master.directory_hq_ext'), true) }}

                {{ textInput($errors, 'text', $directoryHQ, 'directory_hq_email', trans('master.directory_hq_email'), true) }}

                {{ textInput($errors, 'text', $directoryHQ, 'directory_hq_sort', trans('master.directory_hq_sort'), true) }}
            ...
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.directory.hq') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
...
{{-- @include('components.js.ajaxdistrict',[
    'scriptTag' => false,
    'inputEmpty' => $directoryHQ->directory_hq_id
    ]) --}}
        ...
        $.ajax({
            ...
            success: function(data) {
                if(data.status=='ok'){
                    ...
                    function () {
                        window.location.href = '{{ route('master.directory.hq') }}';
                    });
                } else {
                    var inputError = [];
