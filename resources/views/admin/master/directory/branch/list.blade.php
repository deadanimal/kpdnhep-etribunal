    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase"> {{ trans('master.directory_branch') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">

        <table id="tableDirectoryBranch" class="table table-striped table-bordered table-hover table-responsive">
        <thead>
          <tr>
            <th style="width: 3%;">{{ trans('new.no') }}</th>
            <th style="width: 15%;">{!! trans('master.directory_branch_state') !!}</th>
            <th style="width: 38%;">{!! trans('master.directory_branch_address') !!}</th>
            <th style="width: 25%;">{!! trans('master.directory_branch_ks_email') !!}</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ trans('master.directory_branch') }}</h4>
            </div>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        var table = $('#tableDirectoryBranch');

        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.directory.branch') }}",
            "deferRender": true,
            ...
            "columns": [
                ...
                { data: "ks", name:"ks", 'orderable' : false, render: function(data, type, row){
                    return $("<div/>").html(data).text();
                }},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('master.add_directory_branch') }} ", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.directory.branch.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.directory_branch') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('new.directory_branch') }}";
...
function deleteDirectoryBranch(id) {
    swal({
        title: "{{ trans('swal.directory_branch_delete') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.directory.branch') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateDirectoryBranch(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.directory.branch') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
