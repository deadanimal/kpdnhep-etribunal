...
{{ Form::open(['route' => 'master.inquiry_method.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.inquiry_method.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="inquiry_method_id" value="{{ $inquiry_method->inquiry_method_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3> {{ trans('new.add_inquiry_method') }} </h3>
        ...
        <div class="portlet light">
            <div class="portlet-title">
                <div class="caption font-dark">
                    <span class="caption-subject font-green-sharp bold uppercase"> {{ trans('new.add_inquiry_method') }} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
                ...
                {{ textInput($errors, 'text', $inquiry_method, 'method_en', __('new.name').' (EN)', true) }}

                {{ textInput($errors, 'text', $inquiry_method, 'method_my', __('new.name').' (MY)', true) }}

                {{ textInput($errors, 'text', $inquiry_method, 'code', trans('master.code'), true) }}
                
            </div>
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.inquiry_method') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status=='ok'){
                ...
                function () {
                    window.location.href = "{{ route('master.inquiry_method') }}";
                });
            } else {
                var inputError = [];
