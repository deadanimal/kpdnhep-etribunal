    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase"> {{ trans('new.inquiry_method') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        <table id="inquiry_method" class="table table-striped table-bordered table-hover table-responsive">
        <thead>
          <tr>
            <th width="3%">{{ trans('new.no') }}</th>
            <th>{{ __('new.name')}} (EN)</th>
            <th>{{ __('new.name')}} (MY)</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ trans('new.inquiry_method_info') }}</h4>
            </div>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        var table = $('#inquiry_method');

        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.inquiry_method') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // {data: 'rownum'},
                { data: "inquiry_method_id", name:"inquiry_method_id", searchable: false},
                { data: "method_en", name:"method_en", 'orderable' : false},
                { data: "method_my", name:"method_my", 'orderable' : false},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('new.add_inquiry_method') }} ", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.inquiry_method.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.inquiry_method') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('new.inquiry_method') }}";
...
function deleteInquiryMethod(id) {
    swal({
        title: "{{ trans('swal.delete_inquiry_method') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.inquiry_method') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateInquiryMethod(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.inquiry_method') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
