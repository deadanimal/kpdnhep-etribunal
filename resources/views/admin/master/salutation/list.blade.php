    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase"> {{ __('master.salutation') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        ...
        <thead>
          <tr>
            ...
            <th>{{ __('master.salutation_en') }}</th>            
            <th>{{ __('master.salutation_my') }}</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('master.salutation_info')}}</h4>
            </div>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.salutation') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // {data: 'rownum'},
                { data: "salutation_id", name:"salutation_id", searchable: false},
                { data: "salutation_en", name:"salutation_en", 'orderable' : false},
                { data: "salutation_my", name:"salutation_my", 'orderable' : false},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ __('master.add_salutation') }}", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.salutation.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.salutation') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('master.salutation') }}";
...
function deleteSalutation(id) {
    swal({
        title: "{{ __('swal.delete_salutation')}}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.salutation') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateSalutation(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.salutation') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
