...
{{ Form::open(['route' => 'master.salutation.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.salutation.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="salutation_id" value="{{ $salutation->salutation_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3> {{ __('master.salutation') }} </h3>
        ...
        <div class="portlet light">
            <div class="portlet-title">
                <div class="caption font-dark">
                    <span class="caption-subject font-green-sharp bold uppercase"> {{ __('master.salutation') }} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
                ...
                {{ textInput($errors, 'text', $salutation, 'salutation_en', __('master.salutation_en'), true) }}

                {{ textInput($errors, 'text', $salutation, 'salutation_my',__('master.salutation_my'), true) }}

            </div>
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.salutation') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status=='ok'){
                ...
                function () {
                    window.location.href = "{{ route('master.salutation') }}";
                });
            } else {
                var inputError = [];
