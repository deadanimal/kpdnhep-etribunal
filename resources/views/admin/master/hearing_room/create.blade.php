...
{{ Form::open(['route' => 'master.hearing_room.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.hearing_room.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="hearing_room_id" value="{{ $hearing_room->hearing_room_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3>  {{ trans('new.hearing_room')}}</h3>
        ...
        <div class="portlet light">
            <div class="portlet-title">
                <div class="caption font-dark">
                    <span class="caption-subject font-green-sharp bold uppercase">  {{ trans('hearing.add_new_room')}} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body ">
                ...
                {{ textInput($errors, 'text', $hearing_room, 'hearing_room', trans('hearing.room_name'), true) }}

                {{ textarea($errors, $hearing_room, 'address', trans('hearing.address'), true) }}

                {{ masterSelect($errors, $venues, $hearing_room, 'hearing_venue_id', 'hearing_venue_id', 'hearing_venue', __('master.hearing_venue') , true) }}

            </div>
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.hearing_room') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status=='ok'){
                ...
                function () {
                    window.location.href = "{{ route('master.hearing_room') }}";
                });
            } else {
                var inputError = [];
