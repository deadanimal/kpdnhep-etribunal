...
{{ Form::open(['route' => 'master.stop_method.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.stop_method.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="stop_method_id" value="{{ $stop_method->stop_method_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3> {{ __('master.stop_method') }} </h3>
        ...
        <div class="portlet light">
            <div class="portlet-title">
                <div class="caption font-dark">
                    <span class="caption-subject font-green-sharp bold uppercase"> {{ __('master.stop_method') }} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
                ...
                {{ textInput($errors, 'text', $stop_method, 'stop_method_en', __('master.stop_method_en') , true) }}

                {{ textInput($errors, 'text', $stop_method, 'stop_method_my',__('master.stop_method_my'), true) }}

            </div>
            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.stop_method') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status=='ok'){
                ...
                function () {
                    window.location.href = "{{ route('master.stop_method') }}";
                });
            } else {
                var inputError = [];
