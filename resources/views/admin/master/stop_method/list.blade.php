    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase"> {{ __('master.stop_method') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        ...
        <thead>
          <tr>
            ...
            <th>{{ __('master.stop_method_en') }}</th>            
            <th>{{ __('master.stop_method_my') }}</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('master.stop_method_info')}}</h4>
            </div>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.stop_method') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // {data: 'rownum'},
                { data: "stop_method_id", name:"stop_method_id", searchable: false},
                { data: "stop_method_en", name:"stop_method_en", 'orderable' : false},
                { data: "stop_method_my", name:"stop_method_my", 'orderable' : false},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ __('master.add_stop_method') }}", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.stop_method.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.stop_method') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('master.stop_method') }}";
...
function deleteStopMethod(id) {
    swal({
        title: "{{ __('swal.delete_stop_method')}}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.stop_method') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateStopMethod(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.stop_method') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
