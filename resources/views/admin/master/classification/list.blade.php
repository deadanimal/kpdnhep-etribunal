        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption font-dark">
                    ...
                    <span class="caption-subject bold uppercase"> {{ trans('new.list_classification') }} </span>
                </div>
                <div class="tools"> </div>
            </div>
                ...
                <form method='get' action=''>
                    <div id="search-form" class="form-inline">
                        <div class="form-group mb10">
                            <label for="claim_category_id">{{ trans('new.category') }}</label>
                            <select id="claim_category_id" class="form-control" name="category" style="margin-right: 10px;">
                                <option value="" selected disabled hidden>-- {{ __('others.all_category') }} --</option>
                                <option value="" >-- {{ __('others.all_category') }} --</option>
                                @foreach($categories as $i=>$category)
                                <option @if(Request::get('category') == $category->claim_category_id) selected @endif value="{{ $category->claim_category_id }}">{{ $category->$category_lang }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group mb10">
            ...
            <div class="portlet-body">
                <table id="tableClasssification" class="table table-striped table-bordered table-hover table-responsive">
                    <thead>
                        <tr>
                            <th width="3%">{{ __('new.no') }}</th>
                            <th>{{ __('master.classification_en') }}</th>            
                            <th>{{ __('master.classification_my') }}</th>
                            <th>{{ __('master.code') }}</th>
                            <th>{{ __('master.category_'.App::getLocale()) }}</th>
                            <th>{{ __('new.created_at') }}</th>
                            <th style="width: 120px;">{{ __('new.action') }}</th>
                        </tr>
                    </thead>
                </table>
            </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ trans('new.classification_info') }}</h4>
            </div>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        var table = $('#tableClasssification');
        ...
        var oTable = table.DataTable({
            ...
            "columns": [
                 // {data: 'rownum'},
                { data: "claim_classification_id", name:"claim_classification_id", searchable: false},
                { data: "classification_en", name:"classification_en", 'orderable' : false},
                { data: "classification_my", name:"classification_my", 'orderable' : false},
                { data: "rcy_id", name:"rcy_id", 'orderable' : false},
                { data: "category", name:"category.category_{{ App::getLocale() }}", 'orderable' : false},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('new.add_classification') }} ", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.classification.create') }}"; 
                    }
                },
                
                {
                    ...
                    title: function(){
                            return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.list_classification') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('new.list_classification') }}";
...
function deleteClassification(id) {
    swal({
        title: "{{ trans('swal.delete_classification') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.classification') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateClassification(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.classification') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
