    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase"> {{ trans('new.occupation') }} </span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        <table id="occupation" class="table table-striped table-bordered table-hover table-responsive">
        <thead>
          <tr>
            <th width="3%">{{ trans('new.no') }}</th>
            <th>{{__('master.occupation_en')}}</th>
            <th>{{__('master.occupation_en')}}</th>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.occupation') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                 // {data: 'rownum'},
                { data: "occupation_id", name:"occupation_id", searchable: false},
                { data: "occupation_en", name:"occupation_en", 'orderable': false},
                { data: "occupation_my", name:"occupation_my", 'orderable' : false},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('new.add_occupation') }} ", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.occupation.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.occupation') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('new.occupation') }}";
...
function deleteOccupation(id) {
    swal({
        title: "{{ trans('new.occupation_delete') }}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.occupation') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateOccupation(id) {
    swal({
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.occupation') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
