...
{{ Form::open(['route' => 'master.occupation.update', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@else
{{ Form::open(['route' => 'master.occupation.store', 'method' => 'POST', 'class' => 'form-horizontal', 'id' => 'submitForm']) }}
@endif
<input type="hidden" name="occupation_id" value="{{ $occupation->occupation_id }}">
<div class="m-heading-1 border-green m-bordered margin-top-10 margin-bottom-10">
    <h3> {{ trans('new.add_occupation') }} </h3>
        ...
        <div class="portlet light">
            <div class="portlet-title">
                <div class="caption font-dark">
                    <span class="caption-subject font-green-sharp bold uppercase"> {{ trans('new.add_occupation') }} </span>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
                {{ textInput($errors, 'text', $occupation, 'occupation_en', __('master.occupation_en') , true) }}
                {{ textInput($errors, 'text', $occupation, 'occupation_my', __('master.occupation_my') , true) }}
            </div>

            <div class="clearfix">
                <div class="col-md-offset-4 col-md-8 mv20">
                    <button type="button" class="btn default" onclick="location.href ='{{ route('master.occupation') }}'"><i class="fa fa-reply mr10"></i>{{ trans('new.back') }}</button>
                    @if(strpos(Request::url(),'edit') !== false)
                    <button type="submit" class="btn green"><i class="fa fa-paper-plane mr10"></i>{{ trans('button.update') }}</button>
                    @else
                    <button type="submit" class="btn green" data-dismiss="modal"><i class="fa fa-paper-plane mr10"></i>{{ trans('button.create') }}</button>
                    @endif
                </div>
            </div>
        </div>
        ...
        $.ajax({
            ...
            success: function(data) {
                if(data.status=='ok'){
                    ...
                    function () {
                        window.location.href = "{{ route('master.occupation') }}";
                    });
                } else {
                    var inputError = [];
