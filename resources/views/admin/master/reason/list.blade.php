    ...
    <div class="portlet light bordered">
      <div class="portlet-title">
          <div class="caption font-dark">
              ...
              <span class="caption-subject bold uppercase">{{ __('master.visitor_reason') }}</span>
          </div>
          <div class="tools"> </div>
      </div>
      <div class="portlet-body">
        ...
        <thead>
          <tr>
            ...
            <th>{{ __('master.visitor_reason_en') }}</th>            
            <th>{{ __('master.visitor_reason_my') }}</th>
...
var TableDatatablesButtons = function () {
    ...
    var initTable1 = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('master.reason') }}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                { data: "visitor_reason_id", name:"visitor_reason_id", searchable: false},
                { data: "reason_en", name:"reason_en", 'orderable' : false},
                { data: "reason_my", name:"reason_my", 'orderable' : false},
            ...
            buttons: [
                {
                text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ __('master.add_visitor_reason') }}", className:"btn blue btn-outline", action:function()
                    {
                        window.location.href = "{{ route('master.reason.create') }}"; 
                    }
                },
                {
                    extend: 'print',
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('master.visitor_reason') }}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('master.visitor_reason') }}";
...
function deleteReason(id) {
    swal({
        title: "{{ __('swal.delete_visitor_reason')}}",
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.reason') }}/"+id+"/delete",
            type: "DELETE",
            data : {
                _token:_token,
                id:id
...
function activateReason(id) {
    ...
    function(){
        ...
        $.ajax({
            url: "{{ route('master.reason') }}/"+id+"/activate",
            type: "POST",
            data : {
                _token:_token,
                id:id
