...
( {{ __('new.until') .' '.date('d/m/Y') }} )<br></h3>
		...
		<table class="border collapse">
			<tr>
                ...
                <th class="bold uppercase border center fit"> {{ __('new.president') }}  </th>
                ...
                ?>
            <tr>
                <td class="uppercase border center fit">{{ $total_row+1 }}</td>
                <td class="uppercase border center fit" style="text-align: left; text-transform: uppercase;">{{ $president->user->name }}</td>
                <td class="uppercase border center fit">{{ count($form4_prez) }}</td>
            </tr>
            	<?php $total_row++; ?>
                @endforeach
            <tr>
                <td class="bold uppercase border center fit" colspan="2">{{__('new.total')}}</td>
                <td class="bold uppercase border center fit">{{ $total_prez }}</td>
            </tr>
		</table>
</body>