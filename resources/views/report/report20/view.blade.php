        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption font-dark">
                    ...
                    <span class="caption-subject bold uppercase">{!! __('new.report20') !!} </span>
                </div>
                <div class="tools"> </div>
            </div>
                ...
                <table class="table table-bordered table-hover data table-export">
                    <thead>
                        <tr>
                            ...
                            <th> {{ __('new.claimant_identification_no') }}  </th>
                            <th> {{ __('new.claimant_nationality') }}</th>
                            <th> {{ __('new.classification') }} </th>
                            <th> {{ __('new.total_claim') }} </th>
                        </tr>
                    ...
                    <tbody>
                        ...
                        <?php 
                            //$case = (clone $claimcase)->where('case_no', '=', 0);
                            //$case = (clone $claimcase)->where('case_status_id', 8)->get();
                            //dd($case);
                        ?>
                        <tr>
                            <td>{{ $index+1 }}</td>
                            <td style="text-align: left; text-transform: uppercase;"> {{ $claimcase->case_no }}</td>
                            <td>{{ $claimcase->claimant_address->name }}</td>
                            <td>{{ $claimcase->claimant_address->identification_no }}</td>
                            <td>{{ $claimcase->claimant_address->nationality->country }} </td>
                            <td>{{ $claimcase->form1->classification ? $claimcase->form1->classification->classification_my : '-' }}</td>
                            <td>RM {{ $claimcase->form1->claim_amount }}</td>
                        </tr>
                        @endforeach
                        @if ( count($claimcases) == 0 )
                        <tr>
                            <td colspan="7"> {{ __('new.no_record') }}</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
...
function exportExcel() {
    location.href = "{{ url('') }}/report/report20/export/excel?{!! http_build_query(request()->input()) !!}";
}
