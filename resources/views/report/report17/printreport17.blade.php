...
{{ __('new.classification_filed')}} {{ __('new.on_year') .' '.$year }} @if( $month ){{ __('new.month').' '.$month->$month_lang }}@endif <br>
( {{ __('new.until') .' '.date('d/m/Y') }} )<br></h3>
	
		<table class="border collapse">
			<tr>
                <th colspan="2" class="bold uppercase border center fit"></th>
            ...
            <tr>
                <th class="bold uppercase border center fit" width="3%"> {{ __('new.no') }} </th>
                <th class="bold uppercase border center fit"> {{ __('new.state') }}  </th>
                @foreach ($months as $month)
                <th class="bold uppercase border center fit"> B </th>
                <th class="bold uppercase border center fit"> P </th>
                @endforeach
                <th class="bold uppercase border center fit"> B </th>
                <th class="bold uppercase border center fit"> P </th>
            </tr>
            ...
            <tr>
                ...
                @foreach ($months as $index => $month)
                    ...
                    ?>
                <td class="uppercase border center fit"> {{ $report_month->sum('b') }} </td>
                <td class="uppercase border center fit"> {{ $report_month->sum('p') }} </td>
                @endforeach
                <td class="uppercase border center fit"> {{ $report->sum('b') }} </td>
                <td class="uppercase border center fit"> {{ $report->sum('p') }} </td>
            </tr>
                @endforeach
            <tr>
                ...
                ?>
                <td class="bold uppercase border center fit"> {{ $report_month->sum('b') }} </td>
                <td class="bold uppercase border center fit"> {{ $report_month->sum('p') }} </td>
                @endforeach
                <td class="bold uppercase border center fit"> {{ $report17->sum('b') }} </td>
                <td class="bold uppercase border center fit"> {{ $report17->sum('p') }} </td>
            </tr>
		</table>
</body>