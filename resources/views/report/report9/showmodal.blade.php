  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title">{{ __('new.list_visitor') }}</h4>
        </div>
                ...
                <table id="stop_notice" class="table table-striped table-bordered table-hover table-responsive list-modal">
                    <thead>
                        <tr>
                            ...
                            <th>{{ trans('new.identification_no') }}</th>
                        </tr>
                    </thead> 
                </table>
            </div>
...
var TableDatatablesButtons = function () {
        ...
        var oTable = table.DataTable({
            "processing": true,
            "serverSide": true,
            // "ajax": "{{-- route('report3-data-form1', ['state_id' => $state_id, 'year' => $year, 'month' => $month]) --}}",
            "ajax": "{!! route('report9-view-dd-data', ['state_id' => $state_id, 'date_start' => $date_start, 'date_end' => $date_end, 'month' => $month]) !!}",
            "deferRender": true,
            "pagingType": "bootstrap_full_number",
            "columns": [
                // {data: 'rownum'},
                { data: 'id', defaultContent: '', 'orderable' : false, 'searchable' : false},
                // { data: "visitor_identification_no", name:"visitor_identification_no", 'orderable' : false, 'render': function ( data, type, full ){
                //         return $("<div/>").html(data).text(); 
                //     }
                // },
                {
                    data: "visitor_identification_no", name:"visitor_identification_no", 'orderable' : false,
                }
            ],
            ...
            buttons: [
            // {
            //     text:"<i class=\"fa fa-plus margin-right-5\"></i> {{ trans('new.register_stop_notice')}} ", className:"btn blue btn-outline", action:function()
            //     {
            //         window.location.href = "{{ route('stopnotice-find') }}"; 
            //     }
            // },
            // {
            //         extend: 'print',
            ...
            //         title: function(){
            //             return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ trans('new.list_stop_notice') }}</span>"; // #translate
            //         },
