        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption font-dark">
                    ...
                    <span class="caption-subject bold uppercase">{{ __('menu.update_hearing_status')}}</span>
                </div>
            </div>
                    ...
                    <div id="search-form" class="form-inline">
                        ...
                        <div class="form-group mb10">
                            <label for="branch">{{ trans('hearing.branch') }}</label>
                                <select id="branch" class="form-control select2" name="branch" data-placeholder="--------" onchange="loadHearing(this)">
                        ...
                        <div class="form-group mb10">
                            <label for="hearing_date">{{ __('new.on')}}</label>
                            <select id="hearing_date" class="form-control select2" name="hearing_date" data-placeholder="--------">
                                <option value="" disabled selected>--{{ trans('hearing.please_choose')}}--</option>
                            </select>
                        </div>
                        <div class="form-group mb10">
...
function loadHearing(item) {
        ...
        $.ajax({
            url: "{{ url('/') }}/branch/"+branch+"/hearings",
            type: 'GET',
            datatype: 'json',
            success: function(data){
                $.each(data.hearings, function(key, hearings) {
                    var dates = hearings.hearing_date.split("/");
                    ...
                    if(hearings.hearing_room === null)
                        $('#hearing_date').append("<option value='" + hearings.hearing_id +"'>" + hearings.hearing_date + " " + hearings.hearing_time + "</option>");
                    else
                        $('#hearing_date').append("<option value='" + hearings.hearing_id +"'>" + hearings.hearing_date + " " + hearings.hearing_time + " (" + hearings.hearing_room.venue.hearing_venue + " : " + hearings.hearing_room.hearing_room + ")</option>");
                });
            },
    ...
    var initTable = function () {
        ...
        var oTable = table.DataTable({
            ...
            "columns": [
                ...
                { data: "hearing_status", name:"hearing_status.hearing_status_{{ App::getLocale() }}", 'orderable' : false},
                { data: "classification", name:"case.form1.classification.classification_{{ App::getLocale() }}", 'orderable' : false},
                { data: "claimant_name", name:"case.claimant_address.name", 'orderable' : false},
                { data: "opponent_name", name:"claimCaseOpponent.opponent_address.name", 'orderable' : false},
            ...
            buttons: [
                {
                    ...
                    title: function(){
                        return "<span style='font-size: 22px'><strong>e-Tribunal V2 | </strong>{{ __('new.status_edit')}}</span>"; // #translate
                    },
                    ...
                    customize: function ( win ) {
                        ...
                        win.document.title = "{{ __('new.status_edit')}}";
