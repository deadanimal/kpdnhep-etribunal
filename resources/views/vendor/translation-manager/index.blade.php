...
<div style="width: 80%; margin: auto;">
    <h1>{{ __('new.trans_mgr') }}</h1>
    <p>{{ __('new.warning_trans') }}</p>
    <div class="alert alert-success success-import" style="display:none;">
        <p>{{ __('new.done_import') }} <strong class="counter">N</strong> {{ __('new.result_import') }}</p>
    </div>
    <div class="alert alert-success success-find" style="display:none;">
        <p>{{ __('new.done_search') }} <strong class="counter">N</strong> {{ __('new.result_search') }}</p>
    </div>
    <div class="alert alert-success success-publish" style="display:none;">
        <p>{{ __('new.done_publish') }} '<?= $group ?>'!</p>
    </div>
    ...
    <p>
        ...
        <form class="form-inline form-import" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
            ...
            <select name="replace" class="form-control">
                <option value="0">{{ __('new.append_trans') }}</option>
                <option value="1">{{ __('new.replace_trans') }}</option>
            </select>
            <button type="submit" class="btn btn-success"  data-disable-with="Loading..">{{ __('new.import_group') }}</button>
        </form>
        <form class="form-inline form-find" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="{{ __('new.sure_scan') }}">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <p></p>
            <button type="submit" class="btn btn-info" data-disable-with="{{ __('new.searching') }}.." >{{ __('new.find_trans') }}</button>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="{{ __('new.sure_publish') }}  '<?= $group ?>? {{ __('new.overwrite') }}.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="{{ __('new.publishing') }}.." >{{ __('new.publish_trans') }}</button>
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-default">{{ __('button.back') }}</a>
            </form>
        <?php endif; ?>
    </p>
    <form role="form">
        ...
        <div class="form-group">
            <select name="group" id="group" class="form-control group-select" placeholder='{{ __('new.choose_group') }}'>
    ...
    <?php if($group): ?>
        <form action="<?= action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <textarea class="form-control" rows="3" name="keys" placeholder="{{ __('new.add_instruction') }}"></textarea>
            <p></p>
            <input type="submit" value="{{ __('new.add_keys') }}" class="btn btn-primary">
        </form>
        <hr>
    <h4>{{ __('new.total') }}: <?= $numTranslations ?>, {{ __('new.changed') }}: <?= $numChanged ?></h4>
    <table class="table">
        <thead>
        <tr>
            <th width="15%">{{ __('new.key') }}</th>
        ...
        <?php foreach($translations as $key => $translation): ?>
            <tr id="<?= $key ?>">
                ...
                <?php foreach($locales as $locale): ?>
                    ...
                    <td>
                        <a href="#edit" class="editable status-<?= $t ? $t->status : 0 ?> locale-<?= $locale ?>" data-locale="<?= $locale ?>" data-name="<?= $locale . "|" . $key ?>" id="username" data-type="textarea" data-pk="<?= $t ? $t->id : 0 ?>" data-url="<?= $editUrl ?>" data-title="{{ __('new.enter_trans') }}"><?= $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' ?></a>
                    </td>
                <?php endforeach; ?>
                <?php if($deleteEnabled): ?>
                    <td>
                        <a href="<?= action('\Barryvdh\TranslationManager\Controller@postDelete', [$group, $key]) ?>" class="delete-key" data-confirm="{{ __('new.sure_del_trans') }} '<?= $key ?>?"><span class="glyphicon glyphicon-trash"></span></a>
                    </td>
                <?php endif; ?>
            </tr>
        <?php endforeach; ?>
