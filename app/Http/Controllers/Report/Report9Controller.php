...
class Report9Controller extends Controller
{
    ...
    public function view(Request $request)
    {
        ...
        return view('report/report9/view', compact('months', 'states', 'visitor', 'date_start', 'date_end', 'state_id', 'state_list'));
    }
...
' . '( ' . strtoupper(__('new.until')) . ' ' . date('d/m/Y') . ' )');
    ...
    public function data(Request $request)
    {
        ...
        $date_start = $request->date_start
            ? Carbon::createFromFormat('d/m/Y', $request->date_start)->startOfDay()->toDateString()
            : date('Y-m-d');
        $date_end = $request->date_end
            ? Carbon::createFromFormat('d/m/Y', $request->date_end)->endOfDay()->toDateString()
            : date('Y-m-d');
        $month = $request->month ?? '';
