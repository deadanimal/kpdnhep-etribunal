...
use App\SupportModel\RecordBook;
...
class Report11Controller extends Controller
{
    /**
    ...
    public function view(Request $request){
        ...
        $recordbooks = RecordBook::with('state')->where('year', $request->year ? $request->year : date('Y'));
        ...
        if( !$request->has('state') ) {
            return redirect()->route('report11-view', ['state' => Auth::user()->ttpm_data->branch->branch_state_id]);
        }
        ...
        return view("report.report11.view", compact ('states', 'recordbooks', 'years', 'months', 'year', 'states_list'));
    }
    ...
    public function export(Request $request){
        ...
        if($request->format == 'pdf') {
            ...
            $this->data['recordbooks'] = $recordbooks;
            $this->data['months'] = $months;
            $this->data['year'] = $year;
            $this->data['request'] = $request;

            //dd($this->data['report2']);
            $pdf = PDF::loadView('report/report11/printreport11', $this->data)->setOrientation('landscape');

            $pdf->setOption('enable-javascript', true);
            return $pdf->download('Laporan11.pdf');
        }
